

	<body>
	<header>
			<div class="header--container--1">
				<div class="header--div0"><img src="{{ asset('assets/img/u43.png') }}" alt="" /></div>
				<div class="header--title"><p>Sistema de Seguimiento de Egresados y Bolsa de Trabajo</p></div>
				<div class="header--div1"><img src="{{ asset('assets/img/utm_u31.png') }}" alt="" /></div>
			</div>
			<div class="header--container--2">
				<div class="header--option-1"><a href="{{route('Egresados_Ofertas')}}"><p>Ofertas Laborales</p></a></div>
				<div class="header--option-2"><a href="{{route('Egresados_Directorio')}}"><p>Directorio de Empresas</p></a></div>
				<div class="header--option-3"><a href="{{route('TabuladorDeSalarios-Egresados')}}"><p>Tabulador de Salarios</p></a></div>
				<div class="header--option-4"><a href="{{route('Eventos')}}"><p>Eventos</p></a></div>
				<div class="header--option-5"><a href="{{route('CasosDeExito-Egresados')}}"><p>Historias de Éxito</p></a></div>
				<div class="header--option-6"><p>Tips y Consejos</p></a></div>
				<div class="header--option-7">
					<div class="header--icons">
						<img src="{{ asset('assets/icons/help_ofertas.svg')}}" alt="" />
						<img src="{{ asset('assets/icons/config_ofertas.svg')}}" alt="" />
						<img src="{{ asset('assets/img/u462.png')}}" alt="" />
					</div>
				</div>
			</div>
		</header>

	